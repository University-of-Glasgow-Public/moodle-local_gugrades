...
/**
 * Test MGU-1511
 * Incorrect points aggregation when only one item in L2+ category
 * (not rolling up to a percentage)
 ...
 */
final class mgu_1511_single_item_test extends \local_gugrades\external\gugrades_aggregation_testcase {
    /**
    ...
    protected function setUp(): void {
        ...
        // Install test schema.
        $this->gradeitemids = $this->load_schema('schema16');
    ...
    /**
     * Test 'normal' sub-category. Just for comfort.
     * 
     * @covers \local_gugrades\external\get_aggregation_page::execute
     * @return void
     */
    public function test_normal_category(): void {
        global $DB;
        ...
        // Get the grade category 'Normal category'.
        $gradecatnormal = $DB->get_record('grade_categories', ['fullname' => 'Normal category'], '*', MUST_EXIST);

        // Get aggregation page.
        $page = get_aggregation_page::execute($this->course->id, $gradecatnormal->id, '', '', 0, true);
        $page = external_api::clean_returnvalue(
            get_aggregation_page::execute_returns(),
            $page
        );

        // Aggregated grade is a percentage.
        $fred = $page['users'][0];
        $this->assertEquals('66.16667', $fred['displaygrade']);
    }

    /**
     * Test 'single item' sub-category. Just for comfort.
     * 
     * @covers \local_gugrades\external\get_aggregation_page::execute
     * @return void
     */
    public function test_single_category(): void {
        global $DB;
        ...
        // Write grades.
        $item1id = $this->get_gradeitemid('Item1');
        ...
        // Get the grade category 'Normal category'.
        $gradecatsingle = $DB->get_record('grade_categories', ['fullname' => 'Single category'], '*', MUST_EXIST);

        // Get aggregation page.
        $page = get_aggregation_page::execute($this->course->id, $gradecatsingle->id, '', '', 0, true);
        $page = external_api::clean_returnvalue(
            get_aggregation_page::execute_returns(),
            $page
        );
        ...
        // Aggregated grade is a percentage.
        $fred = $page['users'][0];
        $this->assertEquals('58.33333', $fred['displaygrade']);

        // Change grade to admingrade and make sure it bubbles up properly. 
        $nothing = write_additional_grade::execute(
            courseid:       $this->course->id,
            gradeitemid:    $item1id,
            userid:         $this->student->id,
        ...
        // Admin grade should just be the same.
        $fred = $page['users'][0];
     ...
     * @return void
     */
    public function xtest_resit_without_percentage(): void {
        global $DB;
