...
class aggregation {
    ...
    public static function get_columns(int $courseid, int $gradecategoryid, bool $skipuserids = false) {
        ...
        foreach ($gradecategories as $gradecategory) {
            $columns[] = (object)[
                ...
                'showweights' => self::show_weights($gradecategoryid),
                'userids' => [],
    ...
    public static function get_users(int $courseid, int $categoryid, string $firstname, string $lastname, int $groupid) {
        ...
        foreach ($users as $user) {
            $user->displayname = fullname($user);
            $user->resitrequired = \local_gugrades\grades::is_resit_required($courseid, $user->id);
    ...
    public static function recurse_tree(int $courseid, int $gradecategoryid, bool $force = false) {
        ...
        $categorynode = (object)[
            ...
            'excludeempty' => (bool)$gcat->aggregateonlygraded,
     ...
     * @param int $userid
     */
    public static function flag_dropped_items(array $items, int $userid) {
        global $DB;
     ...
     * @param int $userid
     */
    public static function record_weights(array $items, int $userid) {
        global $DB;
    ...
    protected static function aggregate_user(
        ...
        // Get correct regulations object for this course.
        $regulation = \local_gugrades\regulations::get_active_regulation($courseid);
        $aggregation = $regulation->get_aggregation($courseid, $category->atype);
