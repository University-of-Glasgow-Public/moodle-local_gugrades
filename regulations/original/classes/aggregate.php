...
class aggregate {
    ...
    public function aggregate_user_category(int $courseid, object $category, array $items, int $level, int $userid) {
        ...
        $aggfunction = $this->strategy_factory($aggmethod);
        ...
        if ($level == 1) {
            $weighted = $this->is_strategy_weighted($aggmethod);
            $completion = $this->completion($items, $weighted);
        }
            ...
            if ($explain != '') {
                if ($admingrade) {
                    ...
                    if (($atype == \local_gugrades\GRADETYPE_SCHEDULEA) || ($atype == \local_gugrades\GRADETYPE_SCHEDULEB)) {
                        ...
                        $roundpoints = $self->round_float($rawgrade);
        ...
        if (!$isresitcategory) {
            ...
            [$admingrade, $items] = $this->pre_process_items($items);
            if ($admingrade) {
                [$displaygrade, ] = \local_gugrades\admingrades::get_displaygrade_from_name($admingrade);
                $explain = $this->get_explain();
            ...
            if ($level >= 2) {
                if ($admingrade = $this->admin_grades_level2($items)) {
                    [$displaygrade, ] = \local_gugrades\admingrades::get_displaygrade_from_name($admingrade);
                    $explain = $this->get_explain();
            ...
            if ($level == 1) {
                if ($admingrade = $this->admin_grades_level1($items, $completion)) {
                    [$displaygrade, ] = \local_gugrades\admingrades::get_displaygrade_from_name($admingrade);
                    $explain = $this->get_explain();
